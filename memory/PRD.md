...
42. **Public Candidate Self-Onboarding via Invite Links (Feb 2026)** -
    ...
    - First name, last name, 10-digit mobile, email
    ...
    On submit, server runs **Gemini OCR** automatically on Aadhaar (front + back combined) and PAN — no button needed. Extracted Aadhaar number, name, DOB, gender, address, pincode and PAN number are auto-filled into the candidate record. OCR failures are non-blocking (record still created with empty fields, `ocr_status` flags surfaced). Bell-icon notifications fire to all HR Admin / Management.
    - Backend: `routes/candidate_invites.py`. HR endpoints: `POST/GET/DELETE /api/candidate-invites`. Public (no auth): `GET/POST /api/public/candidate-invite/{token}` and `/{token}/submit`.
    - Frontend: new `/apply/:token` page (`pages/CandidateApply.js`) + HR-side modal (`components/candidates/CandidateInvitesModal.js`).
    - Verified end-to-end: invite generated, public submit creates candidate (source=`self_onboarding`), invite marked `used`, re-submission returns HTTP 410.
