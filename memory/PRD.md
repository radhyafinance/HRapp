...
24. **Convert Candidate → Employee** - One-click promotion of a Selected candidate into an Employee record with optional salary / role / bank fields. Backend endpoint `POST /api/candidates/{id}/convert-to-employee` validates uniqueness, copies all KYC (Name, DOB, Gender, Father/Husband, Aadhaar, PAN, Address, Pincode, City, State), copies the candidate's Aadhaar/PAN images to `employee_documents`, creates a `users` record with `Welcome@123` (or custom) password, initializes leave balances (CL=7, SL=15, EL=12), and marks the candidate as `converted` with a back-reference to the new employee `_id`. Frontend exposes a green "Convert to Employee" button below "Download Joining Kit", with a collapsible salary/bank/role/password form.
