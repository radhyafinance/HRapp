...
42. **Manager Role — Employees Page Restrictions (Feb 2026)** -
    `managers` role can no longer Bulk Upload, download/upload templates, Add Employees, or view salary/CTC details of any employee (including their direct reports). **Update (same session):** the `/api/employees` list endpoint now scopes managers to their **full reporting sub-tree** (direct + indirect reports, walked via `services/hierarchy.get_descendant_employee_ids`), not just direct reports. Verified with RMF0010 (Divisional Manager) — list returns 25 employees including transitive descendants (e.g., RMF0036 reporting to RMF0017 reporting to RMF0010). Salary/CTC are still stripped from all employee responses for managers; `/payroll/employee/{other}` and `/payroll/export/neft` continue to return 403 for managers, so payroll data stays invisible.
    - **Backend** `/app/backend/routes/employees.py`: new helper `_strip_salary_for_managers()` removes `salary`, `ctc_monthly`, `ctc_annual` from list & detail responses when caller role is `managers`. List endpoint scope updated to use `get_descendant_employee_ids` (transitive).
    - **Frontend** `/app/frontend/src/pages/Employees.js`: gated the **Bulk Upload**, **Template**, **Add Employee** buttons behind `canManageEmployees = ["hr_admin","management"].includes(role)`.
    - **Frontend** `/app/frontend/src/components/employees/EmployeeModal.js`: **Edit** tab hidden for `managers`. View / Documents / Tracker remain accessible.
