 ...
 *    we need.
 *
 * 2. TRACKING — for employees whose tracker is ACTIVE, requires Android's
 *    "Allow all the time" location. Without it the duty-cycle tracker silently
 *    produces no fixes once the screen locks, so the employee looks tracked
 *    while no data ever arrives. Those employees are held on a block screen
 *    until they grant it. Everyone else — head office, i.e. anyone whose
 *    tracker HR has switched off — is unaffected.
 *
...
// Whether an employee is tracked changes about once in their employment, so one
// lookup per browser session is plenty and keeps the gate off the hot path.
const TRACKED_CACHE_KEY = "rmf_tracking_required";
...
/**
 * Is this employee actually tracked? Read from their own tracker config, which
 * HR controls per person from Field Tracking → Devices.
 *
 * Anything unexpected returns false (= not tracked = allowed through). Accounts
 * with no linked employee record get a 400 from this endpoint, which lands in
 * the same catch and is exactly the behaviour we want.
 */
async function isTrackingRequired() {
  try {
    const cached = sessionStorage.getItem(TRACKED_CACHE_KEY);
    if (cached === "yes") return true;
    if (cached === "no") return false;
  } catch (e) { /* sessionStorage unavailable — just re-fetch */ }
  try {
    const res = await API.get("/tracker/my-tracker");
    const active = Boolean(res && res.data && res.data.active);
    try { sessionStorage.setItem(TRACKED_CACHE_KEY, active ? "yes" : "no"); } catch (e) { /* noop */ }
    return active;
  } catch (e) {
...
function quickVerdict() {
  try {
    ...
    if (sessionStorage.getItem(TRACKED_CACHE_KEY) === "no") return "allowed";
  } catch (e) {
    return "allowed";
  }
  return "checking";
...
async function evaluate() {
  try {
    ...
    if (!(await isTrackingRequired())) return "allowed";
    return "blocked-app";
  } catch (e) {
    return "allowed";
  }
...
function signOut() {
  try {
    ...
    sessionStorage.removeItem(TRACKED_CACHE_KEY);
  } catch (e) { /* noop */ }
  ...
  React.useEffect(() => {
    ...
    const poll = setInterval(() => {
      ...
      if (t !== tokenRef.current) {
        tokenRef.current = t;
        try { sessionStorage.removeItem(TRACKED_CACHE_KEY); } catch (e) { /* noop */ }
        run();
      }
    }, 2000);
    return () => {
