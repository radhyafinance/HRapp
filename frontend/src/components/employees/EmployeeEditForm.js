...
function ResetPasswordSection({ employeeId }) {
  ...
  const reset = async () => {
    setMsg(""); setErr("");
    if (!PWD_RE.test(pwd)) { setErr(`Password must have ${PWD_HINT}.`); return; }
    setBusy(true);
    try {
    ...
    <div className="border-t pt-4">
      ...
      {!show ? (
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 space-y-2">
          ...
          <input type="text" value={pwd} onChange={(e) => setPwd(e.target.value)} placeholder="Min 8 chars, 1 uppercase, 1 number"
          ...
          {pwd && <p className={`text-[10px] mt-0.5 ${PWD_RE.test(pwd) ? "text-green-600" : "text-amber-600"}`}>{PWD_RE.test(pwd) ? "Looks good" : `Needs: ${PWD_HINT}`}</p>}
